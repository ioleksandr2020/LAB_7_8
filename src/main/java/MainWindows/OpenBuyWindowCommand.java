...
public class OpenBuyWindowCommand implements ShopTavernApp.Command {
    ...
    @Override
    public void execute() {
        final Logger logger = LogManager.getLogger(OpenBuyWindowCommand.class);
            ...
            keyFrame = new KeyFrame(frameTime.multiply(i), event -> {
                ...
                if (finalI == message.length() - 1) {
                    ...
                    armorButton.setOnAction(e -> {
                        logger.info("Armor button clicked");
                        openBuyArmorWindow.execute();
                    });

                    weaponButton.setOnAction(e -> {
                        logger.info("Weapon button clicked");
                        openBuyWeaponWindow.execute();
                    });

                    petsButton.setOnAction(e -> {
                        logger.info("Pets button clicked");
                        openPetsWeaponWindow.execute();
                    });

                    cartButton.setOnAction(e -> {
                        logger.info("Cart button clicked");
                        openCartWindow.execute();
                    });
